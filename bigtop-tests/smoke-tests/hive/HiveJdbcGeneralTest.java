...
public class HiveJdbcGeneralTest extends TestMethods {
	...
	FileNotFoundException, ParserConfigurationException, SAXException {
		...
		//String jdbcDriver = propertyValue("hive-site.xml", "javax.jdo.option.ConnectionDriverName");
		...
		con = DriverManager
				.getConnection(jdbcConnection+":"+hivePort,
						connectionProps);
		...
		int columnVerificationNumber = 3;  //The column used to verify the result set data
		...
		assertEquals(printResults(stmt, "Select * from btest", columnVerificationNumber), "20:22");
		assertEquals(printResults(stmt, "Select * from btest where Dest = 'LAX' order by boarded desc", columnVerificationNumber), "7:10");
		assertEquals(printResults(stmt, "Select * from btest where boarded between 160 and 180 order by boarded asc", columnVerificationNumber), "12:19");
		assertEquals(printResults(stmt, "Select * from btest where Dest='LAX' or Dest='ORD'", columnVerificationNumber), "10:43");
		assertEquals(printResults(stmt, "Select * from btest where Dest= 'LAX' and boarded >= 180", columnVerificationNumber), "7:10");
		assertEquals(printResults(stmt, "Select * from btest where Dest ='LAX' and boarded = 197", columnVerificationNumber), "7:10");
		dropTable(stmt, newTableName);
