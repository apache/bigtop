...
usage: $0 <options>
  Required not-so-options:
     --build-dir=DIR             path to celeborn
      dist.dir
     --prefix=PREFIX             path to install into

  Optional options:
     --lib-dir=DIR               path to install celeborn
      home [/usr/lib/celeborn
      ]
     --bin-dir=DIR               path to install bins [/usr/bin]
     --etc-celeborn
     =DIR             path to install celeborn
      conf [/etc/celeborn
      ]
...
NP_VAR_LIB_CELEBORN_DATA=/var/lib/celeborn
/data
...
tar -zxf $BUILD_DIR/build/celeborn-release*.tar.gz -C $TMP_DIR

cp -ra ${TMP_DIR}/celeborn-*/bin/* ${PREFIX}/${LIB_DIR}/bin/
cp -ra ${TMP_DIR}/celeborn-*/charts/* ${PREFIX}/${LIB_DIR}/charts/
cp -ra ${TMP_DIR}/celeborn-*/docker/* ${PREFIX}/${LIB_DIR}/docker/
cp -ra ${TMP_DIR}/celeborn-*/flink/* ${PREFIX}/${LIB_DIR}/flink/
cp -ra ${TMP_DIR}/celeborn-*/jars/* ${PREFIX}/${LIB_DIR}/jars/
cp -ra ${TMP_DIR}/celeborn-*/master-jars/* ${PREFIX}/${LIB_DIR}/master-jars/
cp -ra ${TMP_DIR}/celeborn-*/mr/* ${PREFIX}/${LIB_DIR}/mr/
cp -ra ${TMP_DIR}/celeborn-*/sbin/* ${PREFIX}/${LIB_DIR}/sbin/
cp -ra ${TMP_DIR}/celeborn-*/spark/* ${PREFIX}/${LIB_DIR}/spark/
cp -ra ${TMP_DIR}/celeborn-*/worker-jars/* ${PREFIX}/${LIB_DIR}/worker-jars/

cp -ra ${TMP_DIR}/celeborn-*/conf/* $PREFIX/$ETC_CELEBORN/conf.dist/
