...
usage: $0 <options>
  ...
  Optional options:
     ...
     --var-lib-hive=DIR          path to install hive home [/var/lib/hive]
     --hcatalog-dir=DIR          path to install hcatalog [/usr/lib/hcatalog]
     --var-lib-hcatalog=DIR      path to install hcatalog [/var/lib/hcatalog]
...
while true ; do
    case "$1" in
        ...
        ;;
        --var-lib-hive)
        VAR_LIB_HIVE=$2 ; shift 2
        ;;
        --hcatalog-dir)
        HCATALOG_DIR=$2 ; shift 2
        ;;
        --var-lib-hcatalog)
        VAR_LIB_HCATALOG=$2 ; shift 2
        ;;
        --examples-dir)
...
VAR_LIB_HIVE=${VAR_LIB_HIVE:-/var/lib/hive}
HCATALOG_DIR=${HCATALOG_DIR:-/usr/lib/hive-hcatalog}
VAR_LIB_HCATALOG=${VAR_LIB_HCATALOG:-/var/lib/hive-hcatalog}
...
ln -s $CONF_DIR $PREFIX/$HIVE_DIR/conf
...
install -d -m 1777 $PREFIX/$VAR_LIB_HIVE/metastore/
...
for conf in `cd $PREFIX/$HCATALOG_DIR/etc ; ls -d *` ; do
  ...
  mv $PREFIX/$HCATALOG_DIR/etc/$conf ${PREFIX}/etc/hive-$conf/conf.dist
  ln -s /etc/hive-$conf/conf $PREFIX/$HCATALOG_DIR/etc/$conf
...
install -d -m 0755 $PREFIX/$VAR_LIB_HIVE
