...
install -m 755 scripts/qfs_checkpoint_prune.py $BIN_DIR
install -m 755 scripts/qfs_log_prune.py $BIN_DIR
...
cd build/release
$PYTHON_PATH ../../src/cc/access/kfs_setup.py install --skip-build --root $PREFIX
if [ ! -z "$PYTHON3_PATH" ]; then
    $PYTHON3_PATH ../../src/cc/access/kfs_setup.py install --skip-build --root $PREFIX
fi
cd ..
