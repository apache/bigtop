...
def setup_ranger_admin(upgrade_type=None):
  ...
  Execute(('ln','-sf', format('{ranger_home}/ews/webapp/WEB-INF/classes/conf'), format('{ranger_home}/conf')),
    ...
    sudo=True)
  Execute(('ln','-sf', format('/etc/hadoop/conf/core-site.xml'),format('{ranger_home}/conf/core-site.xml')),sudo=True)
...
def setup_ranger_db(stack_version=None):
  ...
  env_dict = {'RANGER_ADMIN_HOME':ranger_home,'HADOOP_CONF_DIR':params.hadoop_conf,'JAVA_HOME':params.java_home}
  if params.db_flavor.lower() == 'sqla':
    env_dict = {'RANGER_ADMIN_HOME':ranger_home,'HADOOP_CONF_DIR':params.hadoop_conf,'JAVA_HOME':params.java_home, 'LD_LIBRARY_PATH':params.ld_lib_path}
...
def setup_usersync(upgrade_type=None):
  ...
  Execute(('ln','-sf', format('/etc/ranger/usersync/conf'),format("/usr/lib/ranger-usersync/conf")),sudo=True)
  Execute(('ln','-sf', format('/etc/hadoop/conf/core-site.xml'),format('{usersync_home}/conf/core-site.xml')),sudo=True)
  generate_logfeeder_input_config('ranger', Template("input.config-ranger.json.j2", extra_imports=[default]))
...
def setup_tagsync(upgrade_type=None):
  ...
  Execute(('cp', '-f', '/usr/lib/hadoop/lib/hadoop-shaded-guava-1.1.1.jar', '/usr/lib/ranger-tagsync/lib/hadoop-shaded-guava-1.1.1.jar'), sudo=True)
  Execute(('ln','-sf', format('/etc/hadoop/conf/core-site.xml'),format('{ranger_tagsync_home}/conf/core-site.xml')),sudo=True)
  Directory(params.ranger_pid_dir,
    mode=0755,
    create_parents=True,
    owner = params.unix_user,
