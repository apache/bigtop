...
def setup_ranger_plugin_jar_symblink(stack_version, service_name, component_list):
  ...
  jar_files = os.listdir(format('/usr/lib/ranger-{service_name}-plugin/lib'))

  for jar_file in jar_files:
    for component in component_list:
      Execute(('ln','-sf',format('/usr/lib/ranger-{service_name}-plugin/lib/{jar_file}'),format('{stack_root}/current/{component}/lib/{jar_file}')),
      not_if=format('ls {stack_root}/current/{component}/lib/{jar_file}'),
      only_if=format('ls /usr/lib/ranger-{service_name}-plugin/lib/{jar_file}'),
      sudo=True)
...
def setup_ranger_plugin_keystore(service_name, audit_db_is_enabled, stack_version, credential_file, xa_audit_db_password,
  ...
  if cred_lib_path_override is not None:
    ...
    cred_lib_path = format('/usr/lib/ranger-{service_name}-plugin/install/lib/*:/usr/lib/hadoop/lib/*')
    Logger.error(cred_lib_path)

  if cred_setup_prefix_override is not None:
    cred_setup_prefix = cred_setup_prefix_override
  else:
    cred_setup_prefix = (format('/usr/lib/ranger-{service_name}-plugin/ranger_credential_helper.py'), '-l', cred_lib_path)
