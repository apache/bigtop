...
%define usr_lib_zookeeper %{prefix}/usr/lib/%{name}
%define var_lib_zookeeper %{prefix}/var/lib/%{name}

%define bin_dir %{prefix}/%{_bindir}
%define man_dir %{prefix}/%{_mandir}
%define doc_dir %{prefix}/%{_docdir}
%define include_dir %{prefix}/%{_includedir}
...
%post
%{alternatives_cmd} --install %{np_etc_zookeeper}/conf %{name}-conf %{np_etc_zookeeper}/conf.dist 30
%__install -d -o zookeeper -g zookeeper -m 0755 %{var_lib_zookeeper}

%preun
if [ "$1" = 0 ]; then
        %{alternatives_cmd} --remove %{name}-conf %{np_etc_zookeeper}/conf.dist || :
fi
...
%files
%defattr(-,root,root)
%config(noreplace) %{np_etc_zookeeper}/conf.dist
