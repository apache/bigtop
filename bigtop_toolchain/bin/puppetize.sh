...
case ${ID}-${VERSION_ID} in
    ...
    rhel-8*)
        ...
        ;;
    openEuler-*)
        dnf -y install hostname curl sudo unzip wget ruby vim patch systemd-devel findutils 'dnf-command(config-manager)' nc initscripts openeuler-lsb psmisc openssl-devel
